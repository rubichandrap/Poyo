...
if (!entryKey) {
	console.error("❌ Could not find entry point in Vite manifest");
	process.exit(1);
}
...
if (!fs.existsSync(generatedAssetsPath)) {
	...
	console.log("🧹 Selective cleanup of old assets in wwwroot/generated...");
	...
	existingFiles.forEach((file) => {
		...
		if (!activeFiles.has(relativePath)) {
			try {
				...
				console.warn(`  ⚠️ Could not delete locked file ${file}:`, err.message);
			}
		}
	});
}
...
console.log(`✅ Synced ${activeFiles.size} active assets to wwwroot`);
