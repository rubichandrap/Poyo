...
4. **Representation parity over byte identity**: For the same normalized controller-produced Page data value, document `window.SERVER_DATA` and descriptor `pageData` are **structurally equal JSON values** (`JsonElement.DeepEquals`), not byte-identical text, since document embedding intentionally escapes HTML-sensitive characters (`\u003C`). A later descriptor request runs the controller again, so time-varying fields can differ from the earlier document request.
...
- **Compatibility & Migration Guidance**: Restricting `this.PoyoPage` to objects is a breaking change for routes previously supplying primitives or arrays. Callers must wrap non-object values in a top-level property (e.g., `{ items = ... }`). Upgrading projects must replace `ViewBag.ServerData` assignments in views with controller `this.PoyoPage(data)` calls, add `@using Poyo.Framework` to `Views/_ViewImports.cshtml`, and update `_Layout.cshtml` to invoke `@Html.PoyoPageData()`. Release notes across all package changelogs will document this breaking seam.
