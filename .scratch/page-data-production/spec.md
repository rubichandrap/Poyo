...
4. **Structural Parity**: Document `window.SERVER_DATA` and descriptor `pageData` promise **structurally equal JSON values** (`JsonElement.DeepEquals`), not byte-identical text, since document embedding intentionally escapes HTML-sensitive characters (`\u003C`).
...
13. As an application developer, I want a Dynamic navigation descriptor request (`X-Poyo-Navigation: 1`) to return a JSON object structurally equal to what the document embeds, so that navigation and initial page load produce identical state in React.
