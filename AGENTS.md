...
6. **Run the publish-time gate** with `pnpm run test:release` after the workflow completes (or at any time): the lockstep unit tests plus the fixture e2e (`scripts/fixture-e2e.test.mjs`), which scaffolds a real project with the local scaffolder, installs `@rubichandrap/poyo` from npm at the release version, and asserts the resolved package and the `usePage` accessor in the built client bundle. The fixture is **red until the version is published** — an unpublished version fails `pnpm install` with a `[RED-UNTIL-PUBLISHED]` diagnostic. Run it pre-publish to confirm the gate works; it goes green only once the release resolves from npm.
