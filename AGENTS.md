...
**Typed manifest**: `poyo generate` emits `src/routes/routes.generated.ts` — the registry as `as const satisfies readonly RouteEntry[]` plus `RouteName`/`RoutePath` literal unions and a `routePath(name)` helper. The file is gitignored and regenerated by `predev`/`prebuild` and every `poyo route` command; `routes.json` stays the only edited source of truth.
...
The client consumes the registry through the runtime route table: `poyo generate` emits `src/routes/routes.generated.ts` (typed manifest — `RouteName`/`RoutePath` unions plus `routePath()`, see §3.7), gitignored and regenerated by `predev`/`prebuild` and every route command, so `routes.json` stays the only edited source of truth.
