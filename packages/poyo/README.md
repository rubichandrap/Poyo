...
The package ships the C# server core as readable source under `server/` (fixed namespace `Poyo.Framework`): `RoutePolicy` (strict registry validation and route mapping), `RouteDefinition`, the universal `RouteAccessFilter` and `SeoPolicyFilter`, `PageResult` (the single result type answering both the document and the `X-Poyo-Navigation` JSON descriptor, with `Vary` set), `PageController`, `ControllerExtensions.PoyoPage(data)` for custom controllers, and the `AddPoyo()` / `MapPoyoRoutes()` registration extensions.
